# Maintenance â€” Archive, Never-Delete, and Database Safety Rules
...
> Never duplicate these rules â€” link here from any `.ai/instruct.md` that needs them.
...
**Rationale**: Deleted files lose history, context, and recovery paths. The cost of preserved disk space is less than the cost of lost knowledge â€” especially for a project where AI reads these files as instructions.
...
**AI Rule**: Never use `rm`, `Remove-Item`, `git rm`, `del`, or any destructive file operation without **explicit user confirmation**. When in doubt â€” move, don't delete.
...
Restoring is trivial â€” copy the file back to the path shown in `.archive/`.
...
> **â†’ [`.dev-docs` Convention](conventions.md#dev-docs-convention)** â€” dev documentation subdirectory structure, `index.md` format, and AI ignore rules for `.old/`.
...
2. Add a deprecation banner at the very top:
   ```markdown
   > âš ï¸ **DEPRECATED** â€” This file is superseded by [`new/path/.ai/instruct.md`](new/path/.ai/instruct.md).
...
These are pre-approved local, reversible actions â€” no confirmation needed:
