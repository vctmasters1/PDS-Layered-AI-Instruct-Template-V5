# Device Directory - Multi-Container Architecture

**Last Updated**: April 16, 2026  
**Architecture**: Board-Centric (Firmware is generic CoreBinary)
...
**Key Principle**: Device firmware is **truly generic** (CoreBinary). Platform-specific initialization is provided by `pds_platform_main.c` in the selected target's `common/` directory. Board pin-cap specs live in `PDS-BoardEditor/boards/<boardId>.json` (flat — no subdirs).
...
```
Host K:\PDS_AutomationSuite\Device\main  →  Container /src/main  (read-only)
Host K:\PDS_AutomationSuite\Device\pds   →  Container /src/pds   (read-only)
```
...
"mounts": [
  ...
  "source=K:/PDS_AutomationSuite/Device/main,target=/src/main,type=bind,readonly",
  "source=K:/PDS_AutomationSuite/Device/pds,target=/src/pds,type=bind,readonly"
]
```
...
```
K:\PDS_AutomationSuite\Device\main\*.*    ← Edit here
K:\PDS_AutomationSuite\Device\pds\*\*.*   ← Edit here
```
...
> **→ `Device/DEV-Container-esp32c3_sm/AI-INSTRUCT.md` § For AI Agents / Build System** — platform-specific build commands and mount paths for ESP32-C3
> **→ `Device/DEV-Container-SILABS/AI-INSTRUCT.md` § For AI Agents / Build System** — platform-specific build commands for Silicon Labs
...
### [PDS-BuildTools/scripts/build_selector.py](../PDS-BuildTools/scripts/build_selector.py)
- Copy source from `/src/` to container-local `/build/` before building
- Ensures clean builds with correct paths

## Verification

> **→ `Device/DEV-Container-esp32c3_sm/AI-INSTRUCT.md` § Verification** — exact verification commands for ESP32-C3 container

General check: confirm `/src/main` is read-only and `/build` is writable after starting the container. See the container-specific AI-INSTRUCT for exact commands.
...
**Status**: ✅ Architecture finalized - source read-only on host, compiler output container-local  
**Scope**: Multi-container architecture rules for Device builds
