/**
 * PDS Function Block — TMC2208 Stepper Driver implementation
 *
 * Identical to the TMC2209 implementation except:
 *   - No StallGuard / CoolStep registers
 *   - No uart_addr (TMC2208 has no multi-node address support)
 *   - UART pin is named PDN_UART on the TMC2208 datasheet
 *
 * The same three registers are written at init (GCONF, IHOLD_IRUN, CHOPCONF).
 * TMC2208 register map for these three is compatible with TMC2209.
 */
...
static esp_err_t _tmc_write_reg(uart_port_t port, uint8_t reg, uint32_t value)
{
    uint8_t buf[8];
    buf[0] = 0x05u;
    buf[1] = 0x00u;           /* TMC2208 has no node address */
    buf[2] = reg | 0x80u;
...
static void _set_period_us(stepper_tmc2208_ctx_t *ctx, float abs_rpm)
{
...
{
    stepper_tmc2208_ctx_t *ctx = (stepper_tmc2208_ctx_t *)arg;
...
static void _tmc_uart_init(stepper_tmc2208_ctx_t *ctx)
{
    ...
    uart_set_pin(TMC_UART_NUM,
                 (int)ctx->settings.pin_uart,
                 (int)ctx->settings.pin_uart,
                 UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);

    uint32_t gconf = ctx->settings.stealthchop ? 0x00000000u : 0x00000004u;
    _tmc_write_reg(TMC_UART_NUM, TMC_REG_GCONF, gconf);
    ...
    _tmc_write_reg(TMC_UART_NUM, TMC_REG_IHOLD_IRUN, ihold_irun);

    uint32_t chopconf = 0x10000053u;
    chopconf &= ~(0x0Fu << 24);
    chopconf |= (_mres(ctx->settings.microstep_divisor) << 24);
    _tmc_write_reg(TMC_UART_NUM, TMC_REG_CHOPCONF, chopconf);

    ctx->state.uart_configured = true;
}

static esp_err_t _configure_pins(const stepper_tmc2208_ctx_t *ctx)
{
    const pds_fb_stepper_tmc2208_settings_t *s = &ctx->settings;
    ...
    pds_comp_handle_t *out_handle)
{
    ...
    stepper_tmc2208_ctx_t *ctx = calloc(1, sizeof(stepper_tmc2208_ctx_t));
    ...
    const esp_timer_create_args_t ta = {
        ...
        .dispatch_method = ESP_TIMER_TASK, .name = "stepper_tmc2208",
    };
...
{
    stepper_tmc2208_ctx_t *ctx = (stepper_tmc2208_ctx_t *)handle;
...
esp_err_t pds_fb_stepper_tmc2208_connect_speed(pds_comp_handle_t handle, const float *speed_ptr)
{
    if (!handle) return ESP_ERR_INVALID_ARG;
    ((stepper_tmc2208_ctx_t *)handle)->pv_speed = speed_ptr;
    return ESP_OK;
}

esp_err_t pds_fb_stepper_tmc2208_connect_enable(pds_comp_handle_t handle, const bool *enable_ptr)
{
    if (!handle) return ESP_ERR_INVALID_ARG;
    ((stepper_tmc2208_ctx_t *)handle)->pv_enable = enable_ptr;
    return ESP_OK;
}

const pds_fb_stepper_tmc2208_state_t *pds_fb_stepper_tmc2208_get_state(pds_comp_handle_t handle)
{
    return &((stepper_tmc2208_ctx_t *)handle)->state;
}
    ...
    pds_comp_handle_t handle, pds_fb_stepper_tmc2208_settings_t *out)
{
    ...
    memcpy(out, &((stepper_tmc2208_ctx_t *)handle)->settings, sizeof(*out));
    return ESP_OK;
}
...
/* ═══════════════════════════════════════════════════════════════════════════
   TMC2208 — Position Mode
   Context mirrors the first 6 members of stepper_tmc2208_ctx_t exactly so
   _configure_pins() and _tmc_uart_init() can be called via safe cast.
   ═══════════════════════════════════════════════════════════════════════════ */

typedef struct {
    pds_fb_stepper_tmc2208_settings_t         settings;   /* [0] matches velocity ctx */
    pds_fb_stepper_tmc2208_state_t            vel_state;  /* [1] for _tmc_uart_init cast */
    ...
    float                                     prev_trigger;
    pds_fb_stepper_tmc2208_position_state_t   state;
} stepper_tmc2208_pos_ctx_t;

static void _pos_step_cb_tmc2208(void *arg)
{
    stepper_tmc2208_pos_ctx_t *ctx = (stepper_tmc2208_pos_ctx_t *)arg;
...
static void _pos_stop_tmc2208(stepper_tmc2208_pos_ctx_t *ctx)
{
    esp_timer_stop(ctx->step_timer);
...
static void _pos_start_tmc2208(stepper_tmc2208_pos_ctx_t *ctx, int32_t target_steps)
{
...
esp_err_t pds_fb_stepper_tmc2208_position_init(
    const pds_fb_stepper_tmc2208_position_settings_t *settings, pds_comp_handle_t *out_handle)
{
    if (!settings || !out_handle) return ESP_ERR_INVALID_ARG;
    stepper_tmc2208_pos_ctx_t *ctx = calloc(1, sizeof(stepper_tmc2208_pos_ctx_t));
    if (!ctx) return ESP_ERR_NO_MEM;
    memcpy(&ctx->settings, settings, sizeof(ctx->settings));
    if (ctx->settings.steps_per_rev == 0)    ctx->settings.steps_per_rev = 200;
    if (ctx->settings.microstep_divisor == 0) ctx->settings.microstep_divisor = 16;
    esp_err_t ret = _configure_pins((const stepper_tmc2208_ctx_t *)ctx);
    if (ret != ESP_OK) { free(ctx); return ret; }
    _tmc_uart_init((stepper_tmc2208_ctx_t *)ctx);
    const esp_timer_create_args_t ta = {
        .callback = _pos_step_cb_tmc2208, .arg = ctx,
        .dispatch_method = ESP_TIMER_TASK, .name = "stepper_tmc2208_pos",
    };
    ret = esp_timer_create(&ta, &ctx->step_timer);
    if (ret != ESP_OK) { free(ctx); return ret; }
    *out_handle = (pds_comp_handle_t)ctx;
...
{
    stepper_tmc2208_pos_ctx_t *ctx = (stepper_tmc2208_pos_ctx_t *)handle;
    if (!ctx) return PDS_COMP_ERROR;
    ctx->state.done_f = 0.0f;
    float trig = ctx->pv_trigger ? *ctx->pv_trigger : 0.0f;
    bool rising = (trig >= 0.5f) && (ctx->prev_trigger < 0.5f);
    ctx->prev_trigger = trig;
    if (!ctx->settings.enabled) { if (ctx->state.moving) _pos_stop_tmc2208(ctx); return PDS_COMP_ACTIVE; }
    if (ctx->state.moving && ctx->steps_remaining <= 0) {
        _pos_stop_tmc2208(ctx); ctx->state.done_f = 1.0f;
    }
    ...
    if (rising && !ctx->state.moving) {
        ...
        if (target != 0) _pos_start_tmc2208(ctx, target);
    }
    return PDS_COMP_ACTIVE;
}
...
{ stepper_tmc2208_pos_ctx_t *ctx = (stepper_tmc2208_pos_ctx_t *)handle;
  if (!ctx) return ESP_ERR_INVALID_ARG; _pos_stop_tmc2208(ctx); return ESP_OK; }

esp_err_t pds_fb_stepper_tmc2208_position_connect_target(pds_comp_handle_t handle, const float *p)
{ stepper_tmc2208_pos_ctx_t *ctx = (stepper_tmc2208_pos_ctx_t *)handle;
  if (!ctx) return ESP_ERR_INVALID_ARG; ctx->pv_target = p; return ESP_OK; }

esp_err_t pds_fb_stepper_tmc2208_position_connect_trigger(pds_comp_handle_t handle, const float *p)
{ stepper_tmc2208_pos_ctx_t *ctx = (stepper_tmc2208_pos_ctx_t *)handle;
...
{ return &((stepper_tmc2208_pos_ctx_t *)handle)->state; }

esp_err_t pds_fb_stepper_tmc2208_position_get_settings(
    pds_comp_handle_t handle, pds_fb_stepper_tmc2208_settings_t *out)
{ if (!handle||!out) return ESP_ERR_INVALID_ARG;
  memcpy(out,&((stepper_tmc2208_pos_ctx_t *)handle)->settings,sizeof(*out)); return ESP_OK; }

esp_err_t pds_fb_stepper_tmc2208_position_set_settings(
    pds_comp_handle_t handle, const pds_fb_stepper_tmc2208_settings_t *settings)
{ stepper_tmc2208_pos_ctx_t *ctx = (stepper_tmc2208_pos_ctx_t *)handle;
  if (!ctx||!settings) return ESP_ERR_INVALID_ARG;
  _pos_stop_tmc2208(ctx); memcpy(&ctx->settings,settings,sizeof(ctx->settings));
  if (ctx->settings.steps_per_rev==0)    ctx->settings.steps_per_rev=200;
  if (ctx->settings.microstep_divisor==0) ctx->settings.microstep_divisor=16;
  _configure_pins((const stepper_tmc2208_ctx_t *)ctx);
  _tmc_uart_init((stepper_tmc2208_ctx_t *)ctx); return ESP_OK; }
