/**
 * PDS Function Block — TMC2209 Stepper Driver (fb_stepper_tmc2209)
 *
 * Velocity-mode stepper controller for the Trinamic TMC2209 driver.
 * GPIO STEP/DIR/ENABLE pin generation identical to A4988 block.
 * When pin_uart >= 0, the driver is configured via single-wire UART
 * (UART TX/RX bridged on one pin) at startup:
 *   - Microstep resolution (CHOPCONF.MRES)
 *   - Run and hold current (IHOLD_IRUN)
 *   - StealthChop / SpreadCycle selection (GCONF.en_SpreadCycle)
 *
 * UART address (0–3) is set by the MS1/MS2 state at power-on
 * (TMC2209 reads these at reset, after which they become I/O).
 *
 * Pipeline example:
 *   fb_ref(speed_rpm=60) → fb_stepper_tmc2209(pin_step=4, pin_dir=5, pin_uart=6)
 *
 * HAL dependencies: pds_gpio, pds_uart (for UART configuration)
 * Pipeline type ID: 0x63
 */

#ifndef PDS_FB_STEPPER_TMC2209_H
#define PDS_FB_STEPPER_TMC2209_H
...
typedef struct {
    ...
    int8_t   pin_uart;           /**< Single-wire UART pin (-1 = standalone mode) */

    uint8_t  uart_addr;          /**< TMC UART node address 0–3 */
    ...
    bool     enabled;
} pds_fb_stepper_tmc2209_settings_t;
...
} pds_fb_stepper_tmc2209_state_t;

/* ── Position Mode ── */
typedef pds_fb_stepper_tmc2209_settings_t pds_fb_stepper_tmc2209_position_settings_t;
...
} pds_fb_stepper_tmc2209_position_state_t;
...
esp_err_t pds_fb_stepper_tmc2209_connect_speed(
    pds_comp_handle_t handle, const float *speed_ptr);

/** Connect an upstream bool as software enable gate. */
esp_err_t pds_fb_stepper_tmc2209_connect_enable(
    pds_comp_handle_t handle, const bool *enable_ptr);

const pds_fb_stepper_tmc2209_state_t *pds_fb_stepper_tmc2209_get_state(
...
esp_err_t pds_fb_stepper_tmc2209_position_init(
    const pds_fb_stepper_tmc2209_position_settings_t *settings,
    pds_comp_handle_t *out_handle);
...
esp_err_t pds_fb_stepper_tmc2209_position_connect_target(
    pds_comp_handle_t handle, const float *target_steps_ptr);

esp_err_t pds_fb_stepper_tmc2209_position_connect_trigger(
...
#endif /* PDS_FB_STEPPER_TMC2209_H */
