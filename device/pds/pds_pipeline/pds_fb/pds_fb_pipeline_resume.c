/**
 * PDS Function Block — Pipeline Resume implementation
 */
...
extern void pds_pipeline_engine_resume_pipeline(uint8_t idx);

typedef struct {
    pds_fb_pipeline_resume_settings_t settings;
    pds_fb_pipeline_resume_state_t    state;
    const float                       *_trigger_ptr;
    bool                              _prev;   /**< Edge detector */
} pipeline_resume_ctx_t;
    ...
    pds_comp_handle_t *out_handle)
{
    ...
    pipeline_resume_ctx_t *ctx = calloc(1, sizeof(pipeline_resume_ctx_t));
...
pds_comp_status_t pds_fb_pipeline_resume_run(pds_comp_handle_t handle)
{
    ...
    pipeline_resume_ctx_t *ctx = (pipeline_resume_ctx_t *)handle;
    ...
    if (ctx->settings.enabled && current && !ctx->_prev) {
        /* Rising edge — resume the target */
        pds_pipeline_engine_resume_pipeline(ctx->settings.pipeline_index);
    }
    ctx->_prev = current;
...
const pds_fb_pipeline_resume_state_t *pds_fb_pipeline_resume_get_state(
    pds_comp_handle_t handle)
{
    return handle ? &((pipeline_resume_ctx_t *)handle)->state : NULL;
}
    ...
    pds_fb_pipeline_resume_settings_t *out)
{
    ...
    memcpy(out, &((pipeline_resume_ctx_t *)handle)->settings, sizeof(*out));
    return ESP_OK;
}
    ...
    const pds_fb_pipeline_resume_settings_t *settings)
{
    ...
    memcpy(&((pipeline_resume_ctx_t *)handle)->settings, settings, sizeof(*settings));
    return ESP_OK;
}
