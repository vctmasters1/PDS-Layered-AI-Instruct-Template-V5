/**
 * PDS Function Block — DRV8825 Stepper Driver implementation
 *
 * Functionally identical to the A4988 implementation.
 * Differences:
 *   - Microstep table follows DRV8825 MODE0/MODE1/MODE2 truth table
 *   - Supports divisor 32 (1/32 step)
 *   - Optional nFAULT monitoring on each run() tick
 *
 * DRV8825 MODE pin truth table:
 *   Divisor  M0  M1  M2
 *     1       0   0   0
 *     2       1   0   0
 *     4       0   1   0
...
typedef struct { uint8_t m0, m1, m2; } drv_ms_t;
static const drv_ms_t s_drv8825_ms_table[] = {
    { 0, 0, 0 }, /* 1   full  */
    { 1, 0, 0 }, /* 2   half  */
    { 0, 1, 0 }, /* 4   1/4   */
    { 1, 1, 0 }, /* 8   1/8   */
...
typedef struct {
    pds_fb_stepper_drv8825_settings_t settings;
    pds_fb_stepper_drv8825_state_t    state;
    const float *pv_speed;
    const bool  *pv_enable;
    esp_timer_handle_t step_timer;
    uint32_t last_run_tick;
} stepper_drv8825_ctx_t;
...
static void _apply_ms_pins(const stepper_drv8825_ctx_t *ctx)
{
    const drv_ms_t *t = &s_drv8825_ms_table[_divisor_index(ctx->settings.microstep_divisor)];
    if (ctx->settings.pin_ms1 >= 0) PDS_GPIO_write((uint32_t)ctx->settings.pin_ms1, t->m0);
    if (ctx->settings.pin_ms2 >= 0) PDS_GPIO_write((uint32_t)ctx->settings.pin_ms2, t->m1);
    if (ctx->settings.pin_ms3 >= 0) PDS_GPIO_write((uint32_t)ctx->settings.pin_ms3, t->m2);
}

static void _stop(stepper_drv8825_ctx_t *ctx)
{
...
static void _set_period_us(stepper_drv8825_ctx_t *ctx, float abs_rpm)
{
    if (abs_rpm < 0.01f || ctx->settings.pin_step < 0) { _stop(ctx); return; }
    if (ctx->settings.pin_enable >= 0)
        PDS_GPIO_write((uint32_t)ctx->settings.pin_enable, 0);

    uint32_t sps = (uint32_t)(abs_rpm * (float)ctx->settings.steps_per_rev *
...
{
    stepper_drv8825_ctx_t *ctx = (stepper_drv8825_ctx_t *)arg;
...
static esp_err_t _configure_pins(const stepper_drv8825_ctx_t *ctx)
{
    const pds_fb_stepper_drv8825_settings_t *s = &ctx->settings;
...
#undef CFG_OUT
    ...
    if (s->pin_fault >= 0)
        PDS_GPIO_configure((uint32_t)s->pin_fault, PDS_GPIO_MODE_INPUT, PDS_GPIO_PULL_UP);
    ...
    pds_comp_handle_t *out_handle)
{
    ...
    stepper_drv8825_ctx_t *ctx = calloc(1, sizeof(stepper_drv8825_ctx_t));
    ...
    const esp_timer_create_args_t ta = {
        ...
        .dispatch_method = ESP_TIMER_TASK, .name = "stepper_drv8825",
    };
...
{
    stepper_drv8825_ctx_t *ctx = (stepper_drv8825_ctx_t *)handle;
    if (!ctx) return PDS_COMP_ERROR;

    /* Check nFAULT */
    if (ctx->settings.pin_fault >= 0) {
        int fv = PDS_GPIO_read((uint32_t)ctx->settings.pin_fault);
        if (fv == 0) { /* active-low fault */
            ctx->state.fault = true;
            _stop(ctx);
...
esp_err_t pds_fb_stepper_drv8825_connect_speed(pds_comp_handle_t handle, const float *speed_ptr)
{
    if (!handle) return ESP_ERR_INVALID_ARG;
    ((stepper_drv8825_ctx_t *)handle)->pv_speed = speed_ptr;
    return ESP_OK;
}

esp_err_t pds_fb_stepper_drv8825_connect_enable(pds_comp_handle_t handle, const bool *enable_ptr)
{
    if (!handle) return ESP_ERR_INVALID_ARG;
    ((stepper_drv8825_ctx_t *)handle)->pv_enable = enable_ptr;
    return ESP_OK;
}

const pds_fb_stepper_drv8825_state_t *pds_fb_stepper_drv8825_get_state(pds_comp_handle_t handle)
{
    return &((stepper_drv8825_ctx_t *)handle)->state;
}
    ...
    pds_comp_handle_t handle, pds_fb_stepper_drv8825_settings_t *out)
{
    ...
    memcpy(out, &((stepper_drv8825_ctx_t *)handle)->settings, sizeof(*out));
    return ESP_OK;
}
...
/* ═══════════════════════════════════════════════════════════════════════════
   DRV8825 — Position Mode
   ═══════════════════════════════════════════════════════════════════════════ */

typedef struct {
    pds_fb_stepper_drv8825_settings_t          settings;    /* offset must match drv8825_ctx_t */
    esp_timer_handle_t                          step_timer;
    volatile int32_t                            steps_remaining;
    const float                                *pv_target;
    const float                                *pv_trigger;
    float                                       prev_trigger;
    pds_fb_stepper_drv8825_position_state_t     state;
} stepper_drv8825_pos_ctx_t;

static void _pos_step_cb(void *arg)
{
    stepper_drv8825_pos_ctx_t *ctx = (stepper_drv8825_pos_ctx_t *)arg;
...
static void _pos_stop(stepper_drv8825_pos_ctx_t *ctx)
{
    esp_timer_stop(ctx->step_timer);
...
static void _pos_start(stepper_drv8825_pos_ctx_t *ctx, int32_t target_steps)
{
    ...
    _apply_ms_pins((const stepper_drv8825_ctx_t *)ctx);
    if (ctx->settings.pin_enable >= 0)
        PDS_GPIO_write((uint32_t)ctx->settings.pin_enable, 0);
...
{
    if (!settings || !out_handle) return ESP_ERR_INVALID_ARG;
    stepper_drv8825_pos_ctx_t *ctx = calloc(1, sizeof(stepper_drv8825_pos_ctx_t));
    if (!ctx) return ESP_ERR_NO_MEM;
    memcpy(&ctx->settings, settings, sizeof(ctx->settings));
    if (ctx->settings.steps_per_rev == 0)    ctx->settings.steps_per_rev = 200;
    if (ctx->settings.microstep_divisor == 0) ctx->settings.microstep_divisor = 1;
    esp_err_t ret = _configure_pins((const stepper_drv8825_ctx_t *)ctx);
    if (ret != ESP_OK) { free(ctx); return ret; }
    const esp_timer_create_args_t ta = {
        .callback = _pos_step_cb, .arg = ctx,
        .dispatch_method = ESP_TIMER_TASK, .name = "stepper_drv8825_pos",
    };
    ret = esp_timer_create(&ta, &ctx->step_timer);
    if (ret != ESP_OK) { free(ctx); return ret; }
    *out_handle = (pds_comp_handle_t)ctx;
...
{
    stepper_drv8825_pos_ctx_t *ctx = (stepper_drv8825_pos_ctx_t *)handle;
    if (!ctx) return PDS_COMP_ERROR;
    ctx->state.done_f = 0.0f;
...
esp_err_t pds_fb_stepper_drv8825_position_stop(pds_comp_handle_t handle)
{
    stepper_drv8825_pos_ctx_t *ctx = (stepper_drv8825_pos_ctx_t *)handle;
    if (!ctx) return ESP_ERR_INVALID_ARG;
    _pos_stop(ctx); return ESP_OK;
}

esp_err_t pds_fb_stepper_drv8825_position_connect_target(pds_comp_handle_t handle, const float *p)
{ stepper_drv8825_pos_ctx_t *ctx = (stepper_drv8825_pos_ctx_t *)handle;
  if (!ctx) return ESP_ERR_INVALID_ARG; ctx->pv_target = p; return ESP_OK; }

esp_err_t pds_fb_stepper_drv8825_position_connect_trigger(pds_comp_handle_t handle, const float *p)
{ stepper_drv8825_pos_ctx_t *ctx = (stepper_drv8825_pos_ctx_t *)handle;
...
{ return &((stepper_drv8825_pos_ctx_t *)handle)->state; }

esp_err_t pds_fb_stepper_drv8825_position_get_settings(
    pds_comp_handle_t handle, pds_fb_stepper_drv8825_settings_t *out)
{ if (!handle||!out) return ESP_ERR_INVALID_ARG;
  memcpy(out,&((stepper_drv8825_pos_ctx_t *)handle)->settings,sizeof(*out)); return ESP_OK; }
...
{ stepper_drv8825_pos_ctx_t *ctx = (stepper_drv8825_pos_ctx_t *)handle;
  ...
  if (ctx->settings.steps_per_rev==0)    ctx->settings.steps_per_rev=200;
  if (ctx->settings.microstep_divisor==0) ctx->settings.microstep_divisor=1;
  _configure_pins((const stepper_drv8825_ctx_t *)ctx); return ESP_OK; }
