/* ph_001.h — Analog pH probe HAL driver (rev 001)
 *
 * Primitive hardware operations for an analog pH electrode connected to the
 * ESP32 built-in ADC. No calibration, no scaling, no power management —
 * this layer owns only ADC configuration and raw ADC reads.
 *
 ...
 *
 * Protocol:
 *   1. ph_001_configure() — configure the ADC channel once at block init
 *   2. Power-on + settling managed by pds_pwr_group (fb block)
 *   3. ph_001_read_raw() — averaged raw ADC counts after settling
 *   4. ph_001_raw_to_mv() — convert raw count to millivolts for calibration
 *
 * Use in pds_fb_sensor_ph.c only.
 */
...
/**
 * Configure the ESP32 ADC channel for a pH probe.
 *
 ...
 */
esp_err_t ph_001_configure(uint8_t adc_channel);

/**
 * Read oversampled raw ADC counts from the pH probe input.
 *
 ...
 */
esp_err_t ph_001_read_raw(uint8_t adc_channel, uint8_t count, int32_t *out_raw);
 ...
 *
 * @param adc_channel  ESP32 ADC channel number
 * @param raw          Raw ADC count from sensor_ph_read_raw()
 * @return Millivolts, or -1 if calibration unavailable
 */
int ph_001_raw_to_mv(uint8_t adc_channel, int32_t raw);
